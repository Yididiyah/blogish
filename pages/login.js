...
import { loginUser, redirectIfAuthenticated, getToken } from '../lib/api/auth';
...
class Login extends Component {
    ...
    if (formValid) {
      ...
      loginUser(email, password)
        .then((result) => {
          ...
          console.log('Response on Login Page:', response);
          if (response.status) {
            // put the jwt token in a cookie to use for future requests
            Cookie.set('token', response.headers.authorization);
            console.log('Response headers set on cookie Login page: ', Cookie.get('token'));
          ...
          if (error.response) {
            console.log(`Error Response Message on Login page: , Response:  ${error.response.data}, Request ${error.request}`);
  ...
  render() {
    const {
      email, password, passwordError, emailError, formErrorMessage, isLoading,
    } = this.state;
    ...
    return (
      <React.Fragment>
        ...
        <S.Login>
          ...
          <h2 className="title grid-item">Sign in to Blog</h2>
          <form className="grid-item signup-form" onSubmit={this.handleLogin} noValidate autoComplete="off">
            <Input
            ...
            <div className="social-wrap">
              ...
              <p className="social-separator">|</p>
          ...
          <div className="register grid-item">
            ...
            <Link href="/signup">
              <a className="signup-button"> Get Started </a>
            </Link>
          </div>
